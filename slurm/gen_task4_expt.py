...
settings = [(lr, wd, hid, lay, repeat) for lr in learning_rates 
            for wd in weight_decays for hid in hiddens 
            for lay in layers for repeat in range(nr_repeats)]

output_file = open(f"{code_dir}/slurm/experiments/"
                   f"{high_level_expt_name}_experiments.txt", "w")
for lr, wd, hid, lay, repeat in settings:
    lay_str = ' '.join([str(ll) for ll in lay])
    expt_name = f"{high_level_expt_name}__{lr}_{wd}_{hid}_{lay_str.replace(' ', '-')}_{repeat}"
